...
const Popper = React.forwardRef<PopoverRef | undefined, Props>((props: Props, ref) => {
    ...
    const renderMenu = () => (
        <div
            ...
            {...attributes.popper}
        >
            <div className={`sl-popover-content`}>{props.menuContent}</div>
            {props.showArrow && renderArrow()}
        </div>
    );

    const renderArrow = () =>
        props.triggerMode !== "rightClick" && (
            <div ref={setArrowElement} className={"sl-popover-arrow"} style={styles.arrow} {...attributes.arrow} />
        );
